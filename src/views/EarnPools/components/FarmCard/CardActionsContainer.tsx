...
import ConnectWalletButton from 'components/ConnectWalletButton1'
...
const CardActions: React.FC<FarmCardActionsProps> = ({ farm, account, addLiquidityUrl, expanded }) => {
  ...
  const renderApprovalOrStakeButton = () => {
    return isApproved ? (
      ...
      <Button mt="8px" disabled={requestedApproval} style={{ borderRadius: '10px', padding: 22 }} onClick={handleApprove}>
        {t('Enable')}
      </Button>
    )
  }
    ...
    <Action>
      {isApproved && stakedBalance.gt(0) ? (
        ...
        <Flex flex={1} justifyContent="space-between" alignItems="center">
          ...
          {!account ? <ConnectWalletButton btnText="Connect" style={{ padding: '12px 16px', borderRadius: '10px', color: 'white', border: '0px', width: 166 }} /> : renderApprovalOrStakeButton()}
        </Flex>
      )}
